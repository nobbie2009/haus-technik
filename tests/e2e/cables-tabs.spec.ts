...
test("Leitungen zeichnen, bemaßen, Endobjekte bewegen und verlustfrei speichern", async ({ page }) => {
  ...
  await expect(page.getByLabel("Startobjekt", { exact: true })).toBeDisabled();
  ...
  await dialog.getByRole("button", { name: "Belegung speichern", exact: true }).click();
  const connected = (await exported(page)).electrical;
  expect(Object.values(connected.cables)[0]!.conductorConnections.length).toBeGreaterThan(0);
  await page.getByRole("button", { name: "Rückgängig", exact: true }).click();
  expect(Object.keys((await exported(page)).electrical.cables)).toHaveLength(0);
  await page.getByRole("button", { name: "Wiederholen", exact: true }).click();
  expect((await exported(page)).electrical).toEqual(connected);
  await page.locator(".object-list > summary").click();
